...
var dbo = db.db("bigchain");
     ...
     dbo.collection('metadata').findOneAndUpdate({"metadata.id": bu}, {$set: {"metadata.doclist": "0"}},{ upsert:true, returnNewDocument : true,sort:{_id:-1}, } ,function(err,doc) {
       if (err) { throw err; }
...
{
res.sendFile(path.join('/home/rd/project/signup-20200411T140609Z-001/signin-signup/docsignfinal.html'));
}
);
// doc end
...
res.sendFile(path.join('/home/rd/project/signup-20200411T140609Z-001/signin-signup/docdetails.html'));
}
else
{
...
MongoClient.connect(url, function(err, db) {
  ...
  var myobj = { fname: fn, lname: ln , email:email ,password:pass ,phone:phone ,dob:dob, gen:gen,emgemail:emgemail};
...
res.sendFile('/home/rd/project/signup-20200411T140609Z-001/signin-signup/patientsignfinal.html');
}
}
else
{
...
//res.render('/home/rd/project/latestpatientprof1/emergency.ejs',{'email':req.session.email});
...
MongoClient.connect(url, function(err, db) {
  ...
  //Find the first document in the customers collection:
var r=[];
   dbo.collection("metadata").find({"metadata.doclist":req.session.docemail}).toArray( function(err, result)
{
    if (err) throw err;
console.log(result);
...
res.render('/home/rd/project/latestpatientprof1/docprofile.ejs',{'docs':result,'r':r,'email':email});
...
MongoClient.connect(url, function(err, db) {
  if (err) throw err;
  var dbo = db.db("project");
  var myobj = { fname: fn, lname: ln , email:email ,password:pass ,phone:phone ,cw:cw, gen:gen,uni:uni,spl:spl,qual:qual,we:we};
...
conn1.postTransactionCommit(txCreateAliceSimpleSigned)
...
MongoClient.connect(url, function(err, db) {
  ...
  dbo.collection("metadata").find({"metadata.email":req.session.email,"metadata.doclist":{$ne:'0'}}).toArray( function(err, result) {
console.log(result);
for(var i=0;i<=result.length-1;i++)
{
...
console.log(rev);
res.render('/home/rd/project/latestpatientprof1/patientrevokeaccess.ejs',{'doc':rev,'email':decrypt(req.session.email)});
});
})
})
...
dbo.collection("metadata").find({'metadata.id':req.body.bu}).sort({_id:-1}).limit(1).toArray( function(err, result) { 

console.log(result);
 res.render('/home/rd/project/latestpatientprof1/revokelist.ejs',{'doc':result,'email':decrypt(req.session.email)});
})
})
})
...
upd(req.session.bu);
revokeAsset(req.session.bu, req.session.key.publicKey, req.session.key.privateKey,req.body.b);
res.redirect('/revoke');
})
...
dbo.collection("assets").findOne({'data.id':req.body.id},( function(err, result) {  
console.log(result);
...
res.render('/home/rd/project/latestpatientprof1/prescrip.ejs',{'email':req.session.docemail});
});
...
dbo.collection("assets").findOne({'data.id':req.session.aid},( function(err, result) {  
...
conn.postTransactionCommit(txCreateAliceSimpleSigned)
        ...
        .then(() => {
                const txTransferBob = driver.Transaction.makeTransferTransaction(
                        ...
                        [driver.Transaction.makeOutput(driver.Transaction.makeEd25519Condition(patient_key.publicKey))],
                        // metadata
                        {datetime: new Date().toString()}
                )

                // Sign with alice's private key
                let txTransferBobSigned = driver.Transaction.signTransaction(txTransferBob, req.session.key.privateKey)
...
var advice=[];
var sugg=[];
dbo.collection("assets").find({'data.presemail':req.session.email}).toArray( function(err, result) {
console.log(result);
console.log(result.length);
for(var i=0;i<=result.length-1;i++)
{
...
db.close();
 res.render('/home/rd/project/latestpatientprof1/preslist.ejs',{'doc':result,'arr':arr,'allergy':allergy,'splcare':splcare,'advice':advice,'suggestion':sugg,'email':decrypt(req.session.email)});
})
}) 
})
...
res.render('/home/rd/project/latestpatientprof1/docprofile.ejs',{'docs':result,'r':r,'email':req.session.docemail});
...
dbo.collection("psignup").findOne({'email':req.body.bemail},( function(err, result) { 
console.log(result);
...
res.render('/home/rd/project/latestpatientprof1/patienthome.ejs',{'fname':fname,'lname':lname,'email':req.session.docemail,'phone':phone,'dob':dob,'gen':gen,'email1':email1,'cy':currentYear,'ny':todaysDate});
})
)
...
dbo.collection("emergency").find({'emgemail':req.session.email}).sort({_id:-1}).limit(1).toArray( function(err, result) { 
...
var mailOptions = {
  ...
  to: result[0].d_email,
  subject: 'HEALTHCHAIN:',
  text: 'The medical records of '+decrypt(result[0].p_email)+'have been recieved.'
};
transporter.sendMail(mailOptions, function(error, info){
  if (error) {
    console.log(error);
 ...
 var dbob = db.db("bigchain");
  //Find the first document in the customers collection:
   dbob.collection("assets").find({"data.email":result[0].p_email}).toArray( function(err, resu)
{
...
getAsset(resu[i].data.id, emgkey.publicKey, emgkey.privateKey,emeremail);


}
 dbo.collection('psignup').findOneAndUpdate({"email":result[0].emgemail}, {$set: {"emergency": "no"}},{ upsert:true, returnNewDocument : true ,sort:{_id:-1},} ,function(err,doc) {
       if (err) { throw err; }
       else { console.log("Updated"); }
});
...
{
res.sendFile('/home/rd/project/latestpatientprof1/form.html');
...
*/
dbo.collection("psignup").find({"email":encrypt(req.body.patemail)}).toArray( function(err, resu)
{
  console.log(resu);

console.log(resu[0].emgemail);
var emgtext="Your dear one is in trouble.Please log in to our application and check it out. ";
var mailOptions = {
  from: 'healtchchain01@gmail.com',
  to: decrypt(resu[0].emgemail),
  subject: 'HEALTHCHAIN:EMERGENCY!!!!',
  text: emgtext,
};
transporter.sendMail(mailOptions, function(error, info){
  if (error) {
  ...
  dbo.collection('psignup').findOneAndUpdate({"email": resu[0].emgemail}, {$set: {"emergency": "yes"}},{ upsert:true, returnNewDocument : true ,sort:{_id:-1},} ,function(err,doc) {
       if (err) { throw err; }
       else { console.log("Updated"); }
var myobj = { p_email:encrypt(req.body.patemail),incident:req.body.incident,d_email:req.session.docemail,emgemail:resu[0].emgemail};
  dbo.collection("emergency").insertOne(myobj, function(err, re) {
...
MongoClient.connect(url, function(err, db) {
  if (err) throw err;
  var dbo = db.db("project");
dbo.collection("dsignup").find({"email":req.session.d_email}).toArray( function(err, resu)
{
console.log(resu);
res.render('/home/rd/project/latestpatientprof1/emergency.ejs',{'email':decrypt(req.session.email),'docs':resu});
})
});
})
//add the router
